    ...
    app.service('helpers', function($http, $q, $timeout, uiGridConstants, icatSchema, topcatSchema){
    	...
    	this.setupIcatGridOptions = function(gridOptions, entityType){
	        ...
	        if(gridOptions.enableDownload || gridOptions.enableConfigureJob) {
                ...
                if (gridOptions.enableConfigureJob) { actionButtons += '<button class="btn btn-success btn-xs" translate="BROWSE.COLUMN.ACTIONS.BUTTON.CONFIGURE_JOB.TEXT" uib-tooltip="{{\'BROWSE.COLUMN.ACTIONS.BUTTON.CONFIGURE_JOB.TOOLTIP.TEXT\' | translate}}" tooltip-placement="top" tooltip-append-to-body="true" ng-click="grid.appScope.configureJob($event, row.entity)" ng-style="{ \'margin-right\':\'3px\' }"></button>' };
